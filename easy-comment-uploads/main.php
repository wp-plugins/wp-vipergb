...
// Core upload form
//JUSTIN MODIFIED:
//1) Changed "method='POST'" to "method='post'", for validation
//2) wp_nonce_field was both echoing and adding to the string, so it was coming out twice
//3) <label for='file'> cannot have attribute 'name'
function ecu_upload_form_core ($prompt='Select File: ') {
	...
	id='uploadform' style='text-align : center'>";
		...
		echo "<label for='file'>$prompt</label>
...
//CHANGED BY JUSTIN: Append /guestbook/ to the path
function ecu_upload_dir_path () {
	$upload_dir = wp_upload_dir ();
	return $upload_dir ['path'] . '/guestbook/'; // . '/comments/';
}

//CHANGED BY JUSTIN: Append /guestbook/ to the path
function ecu_upload_dir_url () {
	$upload_dir = wp_upload_dir ();
	return $upload_dir ['url'] . '/guestbook/'; // . '/comments/';
}
...
//JUSTIN CHANGED THIS: Remove ecu's defaultfilters
/*
